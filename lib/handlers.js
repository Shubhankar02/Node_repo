    ...
    if (phone) {
        ...
        handlers._tokens.verifyToken(token, phone, (tokenIsValid) => {
            if (tokenIsValid) {
                ...
                callback(403, {'Error': 'Missing the required token in the header'});
            };
        });
...
if(protocol && url && successCodes && timeoutSeconds){
    ...
    const token = typeof(data.payload.token) == 'string' ? data.headers.token : false;
    ...
    callback(400, {'Error': 'Missing inputs or invalid'});
}
