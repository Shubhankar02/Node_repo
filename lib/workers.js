...
workers.processCheckOutcome = function (originalCheckData, checkOutCome) {
    ...
    _data.update('checks', newCheckData.id, newCheckData, (err) => {
        if (!err) {
            ...
            debug('Error trying to save updates in one of the checks');
        };
    });
};
...
workers.alertUserToStatusChange = function (newCheckData) {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, (err) => {
        if (!err) {
            ...
            debug('Error: Could not send the SMS alert to the user who had a state change in their check', err);
        }
    });
};
                ...
                _logs.compress(logId, newFileId, function(err) {
                    if(!err) {
                        ...
                        _logs.truncate(logId, function(err){
                            if(!err) {
                                debug('Sucess trucating the log file');
                            } else {
                                debug('Error tructing the log file');
                            }
                        });
                    } else {
