...
workers.processCheckOutcome = function (originalCheckData, checkOutCome) {
    // Decide if the check consider up and down 
    const state = !checkOutCome.error && checkOutCome.responseCode && originalCheckData.successCodes.indexOf(checkOutCome.responseCode) > -1 ? 'up' : 'down';

    // Decide if the alert is warrented
    // We are deciding whether it is okay to send an sms or alert the user
    // Default state is down, so if there is lastchecked and original state is different, then only alert the user
    ...
    _data.update('checks', newCheckData.id, newCheckData, (err) => {
        if (!err) {
            // Send the newCheckData to the next phase if needed, which means
            ...
            console.log('Error trying to save updates in one of the checks');
        };
    });
};
...
workers.alertUserToStatusChange = function (newCheckData) {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, (err) => {
        if (!err) {
            ...
            console.log('Error: Could not send the SMS alert to the user who had a state change in their check', err);
        }
    });
};
...
workers.log = function (originalCheckData, checkOutCome, state, alertWarranted, timeOfCheck) {
    ...
    // Convert the logData into string
    const logString = JSON.stringify(logData);

    // Determine the name of the log file
    const logFileName = originalCheckData.id

    // Append the logString to the file we want the write to
                ...
                _logs.compress(logId, newFileId, (err) => {
                    if(!err) {
                        ...
                        _logs.trucate(logId, (err)=>{
                            if(!err) {
                                console.log('Sucess trucating the log file');
                            } else {
                                console.log('Error tructing the log file');
                            }
                        });
                    } else {
